...
export function createEmbedder(env: Env): Embedder {
  ...
  if (env.PROVIDER === 'siliconflow') {
    return {
      async embed(texts, dim) {
        const baseUrl = (env.SILICONFLOW_BASE || 'https://api.siliconflow.cn/v1').replace(/\/+$/, '');
        const url = `${baseUrl}/embeddings`;
        const model = env.SILICONFLOW_EMBED_MODEL || 'BAAI/bge-m3';

        console.log('=== SILICONFLOW EMBEDDING REQUEST DEBUG ===');
        console.log('URL:', url);
        console.log('Model:', model);
        console.log('Texts count:', texts.length);
        ...
        if (!resp.ok) {
          const errorText = await resp.text();
          console.error('SiliconFlow embed error:', resp.status, errorText);
          throw new Error(`SiliconFlow embed error: ${resp.status} - ${errorText}`);
        }
